...
namespace MotinGames
{
	...
	public class MotinArrayToolbarEditor  : MotinArrayBaseEditor {
		...
		protected override void objectListUpdated ()
		{
			...
			if(motinDataList.Count ==0)
			{
				toolbarTexts = new string[0];
			}
			else
			{
				toolbarTexts = new string[motinDataList.Count];
				for(int i =0 ; i < motinDataList.Count;i++)
				{
					toolbarTexts[i] = (i+1).ToString();
				}
			}
		...
		public MotinArrayToolbarEditor( ):base()
		{
			handleId = MotinEditorUtils.GetUniqueInteger();
		}
		public MotinArrayToolbarEditor(System.Type type ):base(type)
		{
			handleId = MotinEditorUtils.GetUniqueInteger();
		}
		public MotinArrayToolbarEditor(EditorWindow host,System.Type type ):base(host,type)
		{
			handleId = MotinEditorUtils.GetUniqueInteger();
		}
		public MotinArrayToolbarEditor(string NamespaceName ):base(NamespaceName)
		{
			handleId = MotinEditorUtils.GetUniqueInteger();
		}
		public MotinArrayToolbarEditor(EditorWindow host,string NamespaceName): base(host,NamespaceName)
		{
			handleId = MotinEditorUtils.GetUniqueInteger();
		}
	...
	*/
		protected override void DrawToolbarButtons ()
		{
			...
			if(toolbarTexts.Length>0)
			{
				int newSelectedIndex = GUILayout.Toolbar(selectedObjectIndex,toolbarTexts);
				if(selectedObjectIndex  !=newSelectedIndex)
				{
					selectedObject = motinDataList[newSelectedIndex];
				}
			}
			DrawCreateMenu();
			//base.DrawToolbarButtons ();
		}
...
*/
				...
				listScrollEditor = GUILayout.BeginScrollView(listScrollEditor,GUILayout.ExpandWidth(true) , GUILayout.Height(motinEditors_[selectedObjectIndex].editorContentRect.height));
				motinEditors_[selectedObjectIndex].Draw(new Rect(0,0,editorRect.width,motinEditors_[selectedObjectIndex].editorContentRect.height) ,false );
