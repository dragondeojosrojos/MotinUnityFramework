...
using System;
using System.IO;
public class MotinAnimatorTimeline : EditorWindow {
	
	/*
	[MenuItem ("Window/Animator Timeline Editor")]
    static void Init () {
		EditorWindow.GetWindow (typeof (MotinAnimatorTimeline));
    }
    */
	
	public delegate void EditorWindowDelegate();
	public EditorWindowDelegate	OnWindowClosed = null;
	#region Declarations	
	public MotinAnimator	motinAnimator=null;
	public MotinAnimatorData motinAnimData;
	public AnimatorData animatorData = null;
	public static MotinAnimatorTimeline window = null;
	
	
	public void setMotinAnimator(MotinAnimator motAnim)
	{
		//if(motinAnimator==null /*|| motinAnimator!=motAnim*/)
		{
			
			motinAnimator = motAnim;
			if(animatorData !=null)
			{
				animatorData.destroy();
				animatorData = null;
				
			}
			animatorData = ScriptableObject.CreateInstance<AnimatorData>();
			motinAnimData = motinAnimator.animatorData;
			animatorData.Load(motinAnimData);
			EditorUtility.SetDirty(animatorData);
			AssetDatabase.SaveAssets();
			
			aData = animatorData;
			aData.isAnimatorOpen = true;
			if(aData.takes==null || aData.takes.Count ==0)
			{
				Debug.Log("Takes null or 0 addnew");
				aData.addTake();
				//aData.addTake();
	...
	// temporary variables
	private bool isPlayMode = false; 		// whether the user is in play mode, used to close MotinAnimatorTimeline when in play mode
	...
	void OnEnable() {
		...
		if(window==null)
		{
			Debug.Log("MOtinTimeline ONEnable");
			this.title = "Animator";
		...
		if(motinAnimator!=null)
			setMotinAnimator(motinAnimator);
		
		if(aData==null && animatorData!=null)
			aData = animatorData;
		
		/*
		if(animatorData==null)
		{
			Debug.Log("ANIMATOR DATA IS NULL");
		}
		*/
	}
	void OnDisable() {
		Debug.Log("MotinTimeline OnDisable");
		window = null;
		if(aData) {
			// preview first frame
	...
	void OnGUI() {
		...
		MotinAnimatorTimeline.loadSkin(oData, ref skin, ref cachedSkinName, position);
		...
		*/
		...
		if(GUI.Button(rectBtnCodeView,"Commit", EditorStyles.toolbarButton)) {
		...
		Rect rectBtnDuplicate = new Rect(rectTakePopup.x+rectTakePopup.width+margin,rectBtnOptions.y,60f,height_button_delete);
		if(GUI.Button(rectBtnDuplicate,"Duplicate", EditorStyles.toolbarButton)) {

			aData.duplicateTake(motinAnimData.takes[aData.currentTake]);
			EditorUtility.SetDirty(aData);
			setDirtyTakes(aData.takes);
			//EditorWindow windowOptions = ScriptableObject.CreateInstance<AMOptions>();
			//windowOptions.Show();
		...
		if(rectBtnDuplicate.Contains(e.mousePosition) && mouseOverElement == (int)ElementType.None) mouseOverElement = (int)ElementType.Button;
		#endregion
		...
		Rect rectBtnRenameTake = new Rect(rectBtnDuplicate.x+rectBtnDuplicate.width+margin,rectLabelCurrentTake.y,width_button_delete,height_button_delete);
		// button
		...
		if(GUI.Button (new Rect(0f,0f,position.width,position.height),"","label") && dragType != (int)DragType.TimelineScrub && dragType != (int)DragType.ResizeAction) {
			...
			if(isRenamingTake) {
				...
				//Debug.Log("Rename Take from " + aData.getCurrentTake().assetPath);
				//Debug.Log("Rename Take to " + aData.getCurrentTake().name);
				//Debug.Log("Asset  " + Path.GetDirectoryName(aData.getCurrentTake().assetPath) + "/" +aData.getCurrentTake().name + ".asset");
				//AssetDatabase.RenameAsset(aData.getCurrentTake().assetPath,aData.getCurrentTake().name);
				//aData.getCurrentTake().assetPath = Path.GetDirectoryName(aData.getCurrentTake().assetPath) + "/" +aData.getCurrentTake().name + ".asset";
				//AssetDatabase.SaveAssets();
	...
	void reloadWindowsAnimatorData()
	{
	...
	void OnUndoRedo() {
		...
		*/
		aData	= MotinAnimatorTimeline.window.animatorData;
		// repaint
		//this.Repaint();
		//repaintBuffer = repaintRefreshRate;
		
		// reload AnimatorData for other windows
		reloadWindowsAnimatorData();
	...
	void OnPlayMode() {
		...
		// exit playmode
			...
			*/
			aData	= MotinAnimatorTimeline.window.animatorData;
			if(aData) aData.inPlayMode = false;
			//this.Repaint();
			//repaintBuffer = repaintRefreshRate;
				...
				if(amTrack.keys.Count > 0)
				 {
					...
					Component newComponent = propertyGameObject.GetComponent(((AMPropertyTrack)amTrack).component.GetType());
					if(newComponent == null && !EditorUtility.DisplayDialog("Data Will Be Lost","You will lose all of the keyframes on track '"+amTrack.name+"' if you continue.", "Continue Anway","Cancel"))
					{
					...
					{
						changePropertyGameObject = false;
						((AMPropertyTrack)amTrack).obj = propertyGameObject;
						((AMPropertyTrack)amTrack).component = newComponent;

						//for(int i=0;i<((AMPropertyTrack)amTrack).keys.Count;i++) {
						//	((AMPropertyKey)amTrack.keys[i]).
						//}
					}
				}
	...
	void addTrackWithGameObject(object trackType, GameObject object_window) {
		...
		switch((int) trackType) {
			...
			case (int)Track.CameraSwitcher:
				if(object_window == null && aData.getCurrentTake().cameraSwitcher!=null) {
					// already exists
		...
		if(oData.quickAdd_Combos.Count > 0) {
			...
			foreach(List<int> combo in oData.quickAdd_Combos) {
				...
				for(int i=0; i<combo.Count;i++) {
					...
					combo_name += TrackNames[combo[i]]+" "+(combo[i] == (int)Track.CameraSwitcher && aData.getCurrentTake().cameraSwitcher!=null ? "(Key) " : "");
	...
	void cancelTextEditting(bool toggleIsRenamingTake = false) {
		...
		if(toggleIsRenamingTake) {
			...
			//Debug.Log("Rename toggleIsRenamingTake");
		} else  {
			//Debug.Log("Rename isRenamingTake");
	...
	void CreateTake()
	{
		//Debug.Log("AnimatorData " + Asset)
		
		string name = "Take"+(aData.takes.Count+1);
		//name = aData.makeTakeNameUnique(name);
		AMTake a = ScriptableObject.CreateInstance<AMTake>();
		a.name = name;
		aData.makeTakeNameUnique(a);

		//a.assetPath = aData.dataPath + "/" + a.name + ".asset";
		MotinEditorUtils.CreateAsset(a.assetPath,a);
		// set defaults
		a.name = name;
		...
		aData.takes.Add(a);
		aData.selectTake (aData.takes.Count-1);
		
		//InitGroup
		AMGroup g= ScriptableObject.CreateInstance<AMGroup>();
		g.init(0);
		g.assetPath = Path.GetDirectoryName(a.assetPath) + "/" + a.name + "_group_"+  g.group_name + ".asset";
		MotinEditorUtils.CreateAsset(g.assetPath,g);
	...
	{
		//string assetPath = aData.getCurrentTake().assetPath;
		//aData.deleteCurrentTake();
		//AssetDatabase.DeleteAsset(assetPath);
		animatorData.deleteCurrentTake();
	}
	*/
	//AMTRACK
	// get action for frame from cache
	/*
	public AMAction getActionContainingFrame(AMTrack track, int frame) {
		for(int i=track.cache.Count-1;i>=0;i--) {
			if(frame >= track.cache[i].startFrame) return track.cache[i];	
		}
		if(track.cache.Count > 0) return track.cache[0];	// return first if not greater than any action
	...
	// get action for frame from cache
	public AMAction getActionForFrame(AMTrack track,int startFrame) {
		foreach(AMAction action in track.cache) {
	...
	//AMPropertyTrack
	void addPropertyTrack()
	{
		AMPropertyTrack newTrack = aData.getCurrentTake().addPropertyTrack(null);
		newTrack.assetPath =Path.GetDirectoryName( aData.getCurrentTake().assetPath) + "/" + aData.getCurrentTake().name + "_track_"+  newTrack.name + ".asset";
		MotinEditorUtils.CreateAsset(newTrack.assetPath,newTrack);
		
	}
	
	//AMGROUP
	public static  AMGroup CreateGroupAsset(AMTake take, int group_id = 0, string group_name = null)
	{
		AMGroup g= ScriptableObject.CreateInstance<AMGroup>();
		g.init(group_id,group_name);
		g.assetPath = Path.GetDirectoryName(take.assetPath) + "/" + take.name + "_group_"+  g.group_name + ".asset";
		MotinEditorUtils.CreateAsset(g.assetPath,g);
		return g;
	}
	public void addGroup(AMTake take) {
		//initGroups();
		AMGroup g = ScriptableObject.CreateInstance<AMGroup>();
