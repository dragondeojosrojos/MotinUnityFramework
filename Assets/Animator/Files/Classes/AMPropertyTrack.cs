...
public class AMPropertyTrack : AMTrack {
	...
	{
		base.Load(trackData);
		valueType = trackData.getInt("valueType");
		obj= (GameObject)trackData.getObject("obj");
		setComponent((Component) trackData.getObject("component"));
		propertyName= trackData.getString("propertyName");
		fieldName= trackData.getString("fieldName");
		methodName= trackData.getString("methodName");
	
		keys.Clear();
		foreach(MotinKeyData keyData in trackData.keys)
		{
			AMPropertyKey key = ScriptableObject.CreateInstance<AMPropertyKey>();
			key.Load((ValueType)valueType, keyData);
			keys.Add(key);
		}
		updateCache();
	}
