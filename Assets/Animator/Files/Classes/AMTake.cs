...
public class AMTake :ScriptableObject {
	...
	public void selectTrack(int index, bool isShiftDown, bool isControlDown) {
		...
		{
			Debug.Log("contextSelectionTracks is null");
		}
	...
	float executeActions_delay;

	public void executeActions(float fromFrame ,string animatorId) {
		    executeActions_delay = fromFrame / (float)frameRate;
			maintainCaches();
			...
			foreach(AMTrack track in trackValues) {
				...
				if(action is AMCameraSwitcherAction) (action as AMCameraSwitcherAction).execute(frameRate, executeActions_delay, (track as AMCameraSwitcherTrack).cachedAllCameras);
				else action.execute(frameRate, executeActions_delay,animatorId);	
				}
			}
	}
	...
	{
	//	Debug.Log ("LOAD TAKE NAME " + takeData.name);
		name = takeData.name;
		frameRate = takeData.frameRate;
		numFrames = takeData.numFrames;
		startFrame = takeData.startFrame;
		endFrame = takeData.endFrame;
		playbackSpeedIndex = takeData.playbackSpeedIndex;
		//a.lsTracks = new List<AMTrack>();
		//a.dictTracks = new Dictionary<int,AMTrack>();
		trackKeys = new List<int>();
		groupKeys = new List<int>();
		groupValues = new List<AMGroup>();
		trackValues = new List<AMTrack>();
		contextSelection = new List<int>();	// list of all frames included in the context selection
		ghostSelection = new List<int>();		// list of all frames included in the ghost selection
		contextSelectionTracks = new List<int>();
		
		/*
		foreach(int i in takeData.trackKeys)
		{
			trackKeys.Add(i);
		}
		*/
		...
		//load groups
		foreach(MotinGroupData grpData in takeData.groupValues)
		{
			
			AMGroup g = ScriptableObject.CreateInstance<AMGroup>();
			g.group_id = grpData.group_id;
			g.group_name =   grpData.group_name;
			groupKeys.Add(g.group_id);
			groupValues.Add (g);
			rootGroup.elements.Add(g.group_id);
		// select new group when it has been created
		...
		//load tracks
		AMTrack tmpTrack;
		foreach(MotinTrackData trackData in takeData.trackValues)
		{
			tmpTrack = LoadTrackData(trackData);
			trackKeys.Add(tmpTrack.id);
			trackValues.Add(tmpTrack);
			addToGroup(tmpTrack.id,trackData.groupId);
			tmpTrack.parentTake = this;
		}
		if(trackValues.Count>0)
		{
			selectTrack(trackValues[0].id,false,false);
			//Debug.Log("Take " + name + " selected " + selectedTrack);
		}
		else
		{
			selectedTrack=-1;
	...
	{
		int trackType = trackData.getInt("trackType");
		AMTrack resultTrack = null;
		switch(trackType)
		{
			case (int)MotinTrackData.TrackTypes.PROPERTY_TRACK:
				AMPropertyTrack propTrack = ScriptableObject.CreateInstance<AMPropertyTrack>();
				propTrack.Load(trackData);
				resultTrack =propTrack;
			break;
			case (int)MotinTrackData.TrackTypes.EVENT_TRACK:
				AMEventTrack eventTrack = ScriptableObject.CreateInstance<AMEventTrack>();
				eventTrack.Load(trackData);
				resultTrack =eventTrack;
			break;
		}
