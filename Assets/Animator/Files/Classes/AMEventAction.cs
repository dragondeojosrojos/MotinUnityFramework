...
public class AMEventAction : AMAction {
	...
	public override void execute(int frameRate, float delay,string animatorId) {
		if(useSendMessage) {
			...
			if(parameters == null || parameters.Count <= 0) AMTween.SendMessage(component.gameObject, AMTween.Hash ("delay", getWaitTime(frameRate,delay), "methodname", methodName,"id",animatorId));
			else {
				AMTween.SendMessage(component.gameObject, AMTween.Hash ("delay", getWaitTime(frameRate,delay), "methodname", methodName, "parameter", parameters[0].toObject(),"id",animatorId));
			}
			return;
		}
