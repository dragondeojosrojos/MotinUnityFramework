...
public class AMTransitionPicker : EditorWindow {
	...
	void OnGUI() {
		setWindowSize();
		this.title = "Fade: "+(oData.time_numbering ? MotinAnimatorTimeline.frameToTime(key.frame,(float)aData.getCurrentTake().frameRate)+" s" : key.frame.ToString());
		// load skin
		...
		GUILayout.BeginHorizontal();
			...
			GUILayout.BeginVertical();
				...
				GUILayout.BeginArea(rectArea);
					...
					GUI.color = MotinAnimatorTimeline.getSkinTextureStyleState("properties_bg").textColor;
					...
					if(GUI.Button(rectPlaybackButton,MotinAnimatorTimeline.getSkinTextureStyleState("nav_skip_forward").background,GUI.skin.GetStyle("ButtonImage"))) {
						percent = 1f;
						updateValue();
					}
					...
					if(isPlaying) playToggleTexture = MotinAnimatorTimeline.getSkinTextureStyleState("nav_stop_white").background;
					else playToggleTexture = MotinAnimatorTimeline.getSkinTextureStyleState("nav_play").background;
					if(GUI.Button(rectPlaybackButton,playToggleTexture,GUI.skin.GetStyle("ButtonImage"))) {
						isPlaying = !isPlaying;
					}
					rectPlaybackButton.x -= (18f+2f);
					// first frame button
					if(GUI.Button(rectPlaybackButton,MotinAnimatorTimeline.getSkinTextureStyleState("nav_skip_back").background,GUI.skin.GetStyle("ButtonImage"))) {
						percent = 0f;
						updateValue();
					}
					#endregion
					...
					GUILayout.BeginHorizontal();
						...
						GUILayout.BeginVertical(GUILayout.Width(width_preview_max));
							...
							if(selectedTransition != (int) AMTween.Fade.None) {
								if(MotinAnimatorTimeline.showEasePicker(track,key,aData)) {
									if(isPlaying) percent = waitPercent*-1f;
								}
							}
							// parameters
				...
				GUILayout.BeginVertical(stylePadding,GUILayout.Width(153f));
					...
					GUILayout.BeginVertical(GUILayout.Height(33f));
						...
						GUILayout.BeginHorizontal();
							...
							if(GUI.Button(rectClearButton,MotinAnimatorTimeline.getSkinTextureStyleState("x").background,GUI.skin.GetStyle("ButtonImage"))) {
								searchString = "";
							}
						GUILayout.EndHorizontal();
						GUILayout.FlexibleSpace();
