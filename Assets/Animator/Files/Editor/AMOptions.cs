...
//	private bool exportAllTakes = false;
	...
	void OnGUI() {
		...
		if(!aData) {
			MotinAnimatorTimeline.MessageBox("Animator requires an AnimatorData component in your scene. Launch Animator to add the component.",MotinAnimatorTimeline.MessageBoxType.Warning);
			return;
		}
		...
		GUILayout.BeginVertical();
			...
			if(tabIndex == (int)tabType.General) {
				...
				GUILayout.BeginHorizontal();
					...
					if(oData.setDisableTimelineActions(GUILayout.Toggle(oData.disableTimelineActions,""))) {
						// save
						EditorUtility.SetDirty(oData);
						MotinAnimatorTimeline.recalculateNumFramesToRender();
					}
				GUILayout.EndHorizontal();
			...
			#region quick add
					...
					for(int j=0;j<oData.quickAdd_Combos.Count;j++) {
						...
						GUILayout.BeginHorizontal();
							...
							for(int i=0; i<oData.quickAdd_Combos[j].Count; i++) {
								if(oData.setQuickAddCombo(j,i,EditorGUILayout.Popup(oData.quickAdd_Combos[j][i],MotinAnimatorTimeline.TrackNames,GUILayout.Width(80f)))) {
							...
							if(GUILayout.Button("+", GUILayout.Width(20f), GUILayout.Height(20f))) {
								oData.quickAdd_Combos[j].Add ((int)MotinAnimatorTimeline.Track.Property);
								oData.flatten_quickAdd_Combos();
								EditorUtility.SetDirty(oData);
							}
						GUILayout.EndHorizontal();
					...
					GUILayout.BeginHorizontal();
						...
						// new combo
						if(GUILayout.Button("+", GUILayout.Width(20f), GUILayout.Height(20f))) {
							oData.quickAdd_Combos.Add(new List<int> {(int)MotinAnimatorTimeline.Track.Property});
							oData.flatten_quickAdd_Combos();
							EditorUtility.SetDirty(oData);
						}
					GUILayout.EndHorizontal();
		...
		/*
				...
				GUILayout.BeginHorizontal(GUILayout.Width(300f));
					...
					if(GUILayout.Button("Import",GUILayout.Width(60f))) {
						MotinAnimatorTimeline.registerUndo("Import Take(s)");
				...
				GUILayout.BeginHorizontal();
					...
					if(GUILayout.Button("Import",GUILayout.Width(60f))) {
						MotinAnimatorTimeline.registerUndo("Import Options");
