...
public class AMEasePicker : EditorWindow {
	...
	void OnGUI() {
		...
		this.title = "Ease: "+(oData.time_numbering ? MotinAnimatorTimeline.frameToTime(key.frame,(float)aData.getCurrentTake().frameRate)+" s" : key.frame.ToString());
	...
	private void setupFilteredCategories() {
		...
		foreach(string category in categories) {
			...
			foreach(string ease in MotinAnimatorTimeline.easeTypeNames) {
				if(category == "All") {
					temp.Add(ease);
