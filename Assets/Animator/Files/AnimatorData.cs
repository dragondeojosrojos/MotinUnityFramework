...
public class AnimatorData : ScriptableObject {
	...
	public void Execute(AMTake take, bool isFrame = true, float value = 0f ) {
		...
		take.executeActions(startFrame,animatorId);
	...
	public void addTake() {
		...
		a.contextSelection = new List<int>();	// list of all frames included in the context selection
		a.ghostSelection = new List<int>();		// list of all frames included in the ghost selection
		a.contextSelectionTracks = new List<int>();
		a.groupKeys = new List<int>();
	...
	public void duplicateTake(MotinTakeData take) {
		...
		string name = takes[currentTake].name + "_copy";
		AMTake a = ScriptableObject.CreateInstance<AMTake>();
		a.Load(take);
		a.name = name;
	...
	{
		//Debug.Log("Load MotinAnimatorData ");
		if(takes!=null)
		{
			Debug.Log("null ");
			foreach(AMTake take in takes)
